...
namespace Protoqol\Quo\Polyfill\Mbstring;
...
final class Mbstring
{
    ...
    public static function mb_decode_numericentity($s, $convmap, $encoding = null)
    {
        if (null !== $s && !is_scalar($s) && !(is_object($s) && method_exists($s, '__toString'))) {
            trigger_error(
                'mb_decode_numericentity() expects parameter 1 to be string, ' . gettype($s) . ' given',
                E_USER_WARNING
            );
        ...
        if (null !== $encoding && !is_scalar($encoding)) {
            trigger_error(
                'mb_decode_numericentity() expects parameter 3 to be string, ' . gettype($s) . ' given',
                E_USER_WARNING
            );
    ...
    public static function mb_chr($code, $encoding = null)
    {
        if (0x80 > $code %= 0x200000) {
            ...
            $s = chr(0xF0 | $code >> 18) . chr(0x80 | $code >> 12 & 0x3F) . chr(0x80 | $code >> 6 & 0x3F) . chr(
                0x80 | $code & 0x3F
            );
        }
    ...
    public static function mb_encode_numericentity($s, $convmap, $encoding = null, $is_hex = false)
    {
        if (null !== $s && !is_scalar($s) && !(is_object($s) && method_exists($s, '__toString'))) {
            trigger_error(
                'mb_encode_numericentity() expects parameter 1 to be string, ' . gettype($s) . ' given',
                E_USER_WARNING
            );
        ...
        if (null !== $encoding && !is_scalar($encoding)) {
            trigger_error(
                'mb_encode_numericentity() expects parameter 3 to be string, ' . gettype($s) . ' given',
                E_USER_WARNING
            );
        ...
        if (null !== $is_hex && !is_scalar($is_hex)) {
            trigger_error(
                'mb_encode_numericentity() expects parameter 4 to be boolean, ' . gettype($s) . ' given',
                E_USER_WARNING
            );
    ...
    {
        return self::mb_convert_case($s[1], MB_CASE_UPPER, 'UTF-8') . self::mb_convert_case(
            $s[2],
            MB_CASE_LOWER,
            'UTF-8'
        );
