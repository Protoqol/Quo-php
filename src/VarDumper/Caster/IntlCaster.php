...
class IntlCaster
{
    ...
    public static function castIntlCalendar(IntlCalendar $c, array $a, Stub $stub, bool $isNested, int $filter = 0)
    {
        $a += [
            ...
            Caster::PREFIX_VIRTUAL . 'time_zone'                  => ($filter & Caster::EXCLUDE_VERBOSE) ? new CutStub(
                $c->getTimeZone()
            ) : $c->getTimeZone(),
        ];
    ...
    public static function castIntlDateFormatter(
        ...
        $a += [
            ...
            Caster::PREFIX_VIRTUAL . 'calendar_object' => ($filter & Caster::EXCLUDE_VERBOSE) ? new CutStub(
                $c->getCalendarObject()
            ) : $c->getCalendarObject(),
            Caster::PREFIX_VIRTUAL . 'time_zone'       => ($filter & Caster::EXCLUDE_VERBOSE) ? new CutStub(
                $c->getTimeZone()
