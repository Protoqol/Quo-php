...
class DateCaster
{
    ...
    public static function castTimeZone(DateTimeZone $timeZone, array $a, Stub $stub, bool $isNested, int $filter)
    {
        ...
        $title     = $location && extension_loaded('intl') ? Locale::getDisplayRegion(
    ...
    public static function castPeriod(DatePeriod $p, array $a, Stub $stub, bool $isNested, int $filter)
    {
        ...
        foreach (clone $p as $i => $d) {
            if (self::PERIOD_LIMIT === $i) {
                ...
                $dates[] = sprintf(
                    ...
                    ($end = $p->getEndDate())
                        ? ceil(
                            ($end->format('U.u') - $d->format('U.u')) / ((int)$now->add($p->getDateInterval())->format(
                                'U.u'
                            ) - (int)$now->format('U.u'))
                        )
                        : $p->recurrences - $i
        ...
        $period = sprintf(
            ...
            ($end = $p->getEndDate()) ? 'to ' . self::formatDateTime(
                $end
            ) . (PHP_VERSION_ID >= 80200 && $p->include_end_date ? ']' : '[') : 'recurring ' . $p->recurrences . ' time/s'
        );
