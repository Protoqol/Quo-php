...
class SplCaster
{
    public const SPL_FILE_OBJECT_FLAGS = [
    ...
    public static function castDoublyLinkedList(SplDoublyLinkedList $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        $a += [
            $prefix . 'mode'   => new ConstStub(
                (($mode & SplDoublyLinkedList::IT_MODE_LIFO) ? 'IT_MODE_LIFO' : 'IT_MODE_FIFO') . ' | ' . (($mode & SplDoublyLinkedList::IT_MODE_DELETE) ? 'IT_MODE_DELETE' : 'IT_MODE_KEEP'),
                $mode
            ),
    ...
    public static function castFileObject(SplFileObject $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (isset($a[$prefix . 'fstat'])) {
            $a[$prefix . 'fstat'] = new CutArrayStub(
                $a[$prefix . 'fstat'],
                ['dev', 'ino', 'nlink', 'rdev', 'blksize', 'blocks']
            );
        }
