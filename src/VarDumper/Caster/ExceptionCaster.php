...
use ErrorException;
use Exception;
use Protoqol\Quo\ErrorHandler\Exception\SilencedErrorContext;
...
class ExceptionCaster
{
    ...
    private static function filterExceptionArray(string $xClass, array $a, string $xPrefix, int $filter): array
    {
        ...
        if (
            isset($a[Caster::PREFIX_PROTECTED . 'message']) && str_contains(
            ...
            $a[Caster::PREFIX_PROTECTED . 'message'] = preg_replace_callback(
                ...
                function ($m) {
                    return class_exists($m[0], false) ? (get_parent_class($m[0]) ?: key(
                        class_implements($m[0])
                    ) ?: 'class') . '@anonymous' : $m[0];
                },
                $a[Caster::PREFIX_PROTECTED . 'message']
            );
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (is_file($f['file']) && 0 <= self::$srcContext) {
                    if (
                        !empty($f['class']) && (is_subclass_of($f['class'], 'Twig\Template') || is_subclass_of(
                            $f['class'],
                            'Twig_Template'
                        ...
                        if (null !== $template) {
                            ...
                            if (isset($templateInfo[$f['line']])) {
                                if (
                                    !method_exists($template, 'getSourceContext') || !is_file(
    ...
    private static function extractSource(
        ...
        if ($frame['function'] ?? false) {
            ...
            try {
                $caller      = isset($frame['class']) ? new ReflectionMethod(
                    $frame['class'],
                    $frame['function']
                ) : new ReflectionFunction($frame['function']);
                $stub->class .= ReflectionCaster::getSignature(
                    ReflectionCaster::castFunctionAbstract($caller, [], $stub, true, Caster::EXCLUDE_VERBOSE)
                );
