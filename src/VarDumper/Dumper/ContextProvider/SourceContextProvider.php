...
use Protoqol\Quo\HttpKernel\Debug\FileLinkFormatter;
    ...
    public function getContext(): ?array
    {
        ...
        for ($i = 2; $i < $this->limit; ++$i) {
            if (
                ...
                while (++$i < $this->limit) {
                    if (
                        isset($trace[$i]['function'], $trace[$i]['file']) && empty($trace[$i]['class']) && !str_starts_with(
                            $trace[$i]['function'],
                            'call_user_func'
                        )
                    ) {
                        ...
                        if (isset($info[$trace[$i - 1]['line']])) {
                            ...
                            if ($src) {
                                ...
                                for ($i = max($line - 3, 1), $max = min($line + 3, count($src)); $i <= $max; ++$i) {
                                    $fileExcerpt[] = '<li' . ($i === $line ? ' class="selected"' : '') . '><code>' . $this->htmlEncode(
                                        $src[$i - 1]
                                    ) . '</code></li>';
                                }
        ...
        if (
            $this->fileLinkFormatter && $fileLink = $this->fileLinkFormatter->format(
                $context['file'],
                $context['line']
            )
        ) {
